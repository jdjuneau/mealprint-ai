 ...
 * This is the same logic but without the auth requirement
 */
export async function generateBriefInternal(userId: string, timeOfDay: 'morning' | 'afternoon' | 'evening'): Promise<{ brief: string }> {
  ...
  try {
    ...
    // Get goals from user document directly (not from subcollection)
    const goals = profile?.goals || {};
    const useImperial = goals?.useImperial === true || profile?.useImperial === true;
    ...
    if (isNewAccount || !hasAnyData) {
      ...
      // NO CACHING - Return welcome brief directly
...
${useImperial ? '\n9. CRITICAL: User has IMPERIAL units. You MUST NEVER use metric measurements. Weight MUST be in lbs (pounds), NEVER kg or kilograms. Height MUST be in feet/inches, NEVER cm or meters. Liquids MUST be in fl oz or cups, NEVER ml or liters. If you see any metric values in the data, convert them: 1 kg = 2.20462 lbs, 1 cm = 0.393701 inches, 1 ml = 0.033814 fl oz, 1 liter = 33.814 fl oz.' : ''}
...
${useImperial ? '\n11. CRITICAL: User has IMPERIAL units. You MUST NEVER use metric measurements. Weight MUST be in lbs (pounds), NEVER kg or kilograms. Height MUST be in feet/inches, NEVER cm or meters. Liquids MUST be in fl oz or cups, NEVER ml or liters. If you see any metric values in the data, convert them: 1 kg = 2.20462 lbs, 1 cm = 0.393701 inches, 1 ml = 0.033814 fl oz, 1 liter = 33.814 fl oz.' : ''}
...
${useImperial ? '- CRITICAL: User has IMPERIAL units. You MUST NEVER use metric measurements. Weight MUST be in lbs (pounds), NEVER kg or kilograms. Height MUST be in feet/inches, NEVER cm or meters. Liquids MUST be in fl oz or cups, NEVER ml or liters. If you see any metric values in the data, convert them: 1 kg = 2.20462 lbs, 1 cm = 0.393701 inches, 1 ml = 0.033814 fl oz, 1 liter = 33.814 fl oz.' : ''}`
        },
    ...
    // Post-process for imperial units - CRITICAL: Remove ALL metric measurements
    if (useImperial && brief) {
      ...
      // Replace ml/milliliters with fl oz (for water/liquids)
      brief = brief.replace(/\b(\d+(?:\.\d+)?)\s*(?:ml|milliliters?|millilitres?)\b/gi, (match, num) => {
        const mlValue = parseFloat(num);
        const flOz = (mlValue * 0.033814).toFixed(1);
        return `${flOz} fl oz`;
      });
      
      // Replace cm/centimeters with inches (for height)
      brief = brief.replace(/\b(\d+(?:\.\d+)?)\s*(?:cm|centimeters?|centimetres?)\b/gi, (match, num) => {
        const cmValue = parseFloat(num);
        const inches = (cmValue / 2.54).toFixed(1);
        return `${inches} inches`;
      });
      
      // Replace meters with feet and inches
      brief = brief.replace(/\b(\d+(?:\.\d+)?)\s*(?:m|meters?|metres?)\b/gi, (match, num) => {
        const metersValue = parseFloat(num);
      ...
      // Replace liters with fl oz or cups
      brief = brief.replace(/\b(\d+(?:\.\d+)?)\s*(?:l|liters?|litres?)\b/gi, (match, num) => {
        const litersValue = parseFloat(num);
        const flOz = (litersValue * 33.814).toFixed(1);
        return `${flOz} fl oz`;
      });
      
      // Remove any remaining metric references
      brief = brief.replace(/\b(?:metric|kilograms?|milliliters?|centimeters?|meters?|liters?)\b/gi, '');
    }
...
/**
 * Generate brief - NO CACHING, always fresh data
 * Supports morning, afternoon, and evening briefs with different content
 */
export const generateBrief = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  // Get time of day from request data, or determine from most recent scheduled brief time
  // Scheduled brief times: Morning 9 AM, Afternoon 2 PM, Evening 6 PM
  const timeOfDay = data?.timeOfDay || (() => {
    const hour = today.getHours();
    const minute = today.getMinutes();
    const timeInMinutes = hour * 60 + minute;
    
    // Determine which scheduled brief was most recently sent:
    // Morning brief sent at 9 AM (540 min)
    // Afternoon brief sent at 2 PM (840 min)
    // Evening brief sent at 6 PM (1080 min)
    // Show the most recent brief that has been sent
    if (timeInMinutes >= 1080) {
      return 'evening'; // After 6 PM, show evening brief
    } else if (timeInMinutes >= 840) {
      return 'afternoon'; // After 2 PM, show afternoon brief
    } else if (timeInMinutes >= 540) {
      return 'morning'; // After 9 AM, show morning brief
    } else {
      return 'evening'; // Before 9 AM, show previous evening brief
    }
  })() as 'morning' | 'afternoon' | 'evening';

  try {
    return await generateBriefInternal(userId, timeOfDay);
  } catch (error: any) {
    console.error('Error generating brief:', error);
    throw new functions.https.HttpsError('internal', 'Failed to generate brief', error.message);
  }
});
